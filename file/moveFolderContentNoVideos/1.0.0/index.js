...
var details = function () { return ({
	id: 'mitsie_copy_move_file_no_more_vids',
    name: 'Copy/Move Folder Content No More Vids',
    description: "Copy or move folder content to another folder only when no more videos are left in folder. \nDoes not apply to the current file being processed (either the original or working file).\nUseful if, for example, you want to move things like subtitle files or cover art to a new folder.",
    style: {
        borderColor: 'green',
    },
    tags: '',
...
const checkVideoFilesRemaining = function(dir){
	var video_files_remaining = 0;
	const fs = require("fs");
	args.jobLog(`Checking Video File Count..`);
    ...
    return __generator(this, function (_c) {
        switch (_c.label) {
            case 0:
				...
				if (checkVideoFilesRemaining() === true) return [3 /*break*/, 1];
				args.jobLog("Video Files Remain, skipping ".concat(operation));
                return [3 /*break*/, 3];
            case 1:
